...
export default function HomePageClient({ clientLogos, steps }: { clientLogos: ClientLogo[]; steps: Step[] }) {
							...
							{clientLogos.map((logo, index) => (
								<GridItem key={index}>
									<Flex
										...
										p={2}
									>
										<Box position="relative" width="80%" maxW="80%" height="80px">
											<NextImage
												src={logo.src}
												alt={logo.alt}
												fill
						...
						<Box flex={{ base: "1 1 100%", md: "1 1 50%" }}>
							<Box bg="brandPurple.600" color="brandNeutral.500" mb="2" px="3" py="2" borderRadius="lg" fontWeight="700" fontSize="md" width="fit-content" mx={{ base: "auto", md: "0" }}>
								<Flex gap="3" direction="row" alignItems="center">
									<NextImage src="/PencilRuler.svg" alt="Pencil Ruler Icon" width={16} height={16} />
